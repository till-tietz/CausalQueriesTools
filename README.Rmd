...
### Motivation 
Complex theories beget complex models; posing substantial computational challenges to the practical application of the `CausalQueries` framework to theories of complexity often favored by practitioners. Even relatively harmless seeming DAGs may begin to test the limits of memory and processing power available on modern hardware once encoded as causal models with standard `CausalQueries`. Consider, by way of illustration, the model described by the following DAG. It alone has ~4 Billion nodal- and ~8796 Billion causal-types, as well as a myriad of further parameters attached to both.     

```{r, echo = FALSE, fig.width=6, fig.height=6, fig.align='center',fig.width=7, fig.height=8,out.height='55%', out.width='50%', fig.asp=0.8}
...
```{r, echo = FALSE, fig.width=7, fig.height=8, fig.align='center', message=FALSE, warning= FALSE,out.height='55%', out.width='50%', fig.asp=0.8}
...
```
\
To illustrate the above points on the performance gains derived through model splitting, we benchmark `update_model()` and `update_stitch()` on updating models of increasing size with data consisting of 1000 observations. Models are simple causal chains with 2 to 7 nodes. We update each models 10 times using each function and present average run-times. We run 4 markov chains in parallel for both `update_model()` and `update_stitch()`, with `update_stitch()` further parallelising across sub-models. 

```{r, echo = FALSE, fig.width=7, fig.height=6, fig.align='center', out.height='55%', out.width='50%', fig.asp=0.8}
